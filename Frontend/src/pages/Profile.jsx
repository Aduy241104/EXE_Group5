...
export default function Profile() {
  ...
  // admin find user (thử nhiều endpoint)
  const adminFindUser = async () => {
    ...
    try {
      const r = await tryMulti({
        paths: [
          `/api/users/admin/find`,
          `/api/admin/users/find`,
          `/api/admin/users`, // ?email=
        ].map((p) => (p.includes("?") ? `${p}&email=${encodeURIComponent(searchEmail.trim())}` : p + `?email=${encodeURIComponent(searchEmail.trim())}`)),
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const adminUpdateRole = async () => {
    ...
    try {
      ...
      // refresh me nếu đổi chính mình
      if (targetUser.id === user?.id) {
        const me = await tryMulti({
          paths: ["/api/auth/me", "/api/users/me", "/api/user/me", "/api/me"],
          headers: { Authorization: `Bearer ${token}` },
        });
        setProfile(me.data);
  ...
  // avatar tuyệt đối theo các base, ưu tiên ENV_BASE
        ...
        <div className="mb-6 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-emerald-800">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm">
              <b>Quyền quản trị:</b> Bạn có thể tìm người dùng theo email và cập nhật <i>role</i>.
            </div>
          </div>
        </div>
      )}
